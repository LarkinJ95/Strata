...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  }, { paper: (query.get("paper") as "letter" | "legal" | "a4" | "a3" | null) ?? "letter", orientation: (query.get("orientation") as "portrait" | "landscape" | null) ?? "portrait", density: (query.get("density") as "standard" | "compact" | null) ?? "standard", nestLayers: query.get("nestLayers") !== "false", groupRepeated: false, includeFloorPlans: (query.get("includeFloorPlans") ?? query.get("plans")) !== "false", includeRemoved: (query.get("includeRemoved") ?? query.get("removed")) === "true", floor: query.get("floor") || undefined, functionalAreaId: query.get("functionalAreaId") || undefined, floorOrder: query.get("floorOrder") === "descending" ? "descending" : "ascending" });
