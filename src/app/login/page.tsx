      ...
      <div className="relative mx-auto grid min-h-screen max-w-6xl items-center gap-10 px-6 py-12 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="text-white">
          <img
            src="/strata-logo.png"
            alt="Strata"
            className="mb-8 h-16 w-auto max-w-full object-contain drop-shadow-[0_0_40px_rgba(40,140,255,0.55)]"
          />
          ...
          <p className="mt-5 max-w-xl text-[15px] leading-7 text-white/70">
            Inventory, sampling, laboratory results, inspections, photographs, repairs, and removals —
            linked into one traceable asbestos record for each building.
          </p>
        </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-white p-7 shadow-[0_30px_80px_-30px_rgba(0,0,0,0.6)]">
          ...
          <form action="/api/auth/session-login" method="post" className="space-y-4">
            ...
            {error && (
              <div className="rounded-lg bg-[#fdecec] px-3 py-2 text-sm text-[#b42318]">
                {error === "invalid" ? "Could not sign in. Try again." : "Email and password are required."}
              </div>
            )}
          ...
          <div className="mt-3 space-y-1.5">
            {DEMOS.map((d) => (
              <form key={d.email} action="/api/auth/session-login" method="post">
                <input type="hidden" name="email" value={d.email} />
                <input type="hidden" name="password" value="Strata2026!" />
                <input type="hidden" name="next" value={next && next.startsWith("/") ? next : ""} />
                <button type="submit" className="flex w-full items-center justify-between rounded-xl px-2 py-2 text-left text-sm hover:bg-teal-soft/60">
                  <span>
