...
export async function DataQualityDashboard({ user }: { user: SessionUser }) {
  ...
  const Block = ({ title, count, children, href }: { title: string; count: number; children: React.ReactNode; href?: string }) => (
    <Panel className="p-5">
      <SectionTitle action={<span className="chip chip-ice">{count}</span>}>{title}</SectionTitle>
  ...
  return <div className="grid gap-4 xl:grid-cols-2">
    ...
    <Block title="Inventory without a functional area" count={unassignedFa.length} href="/inventory">{unassignedFa.slice(0, 6).map((item) => <Link key={item.id} href={`/inventory/${item.id}`} className="block rounded-lg px-2 py-1.5 hover:bg-paper-2"><b>{item.inventoryCode}</b> · {item.materialDescription}<span className="ml-2 text-xs text-ink-3">{item.building.buildingNumber}</span></Link>)}</Block>
    <Block title="Inventory without a floor" count={missingFloor.length} href="/inventory">{missingFloor.slice(0, 6).map((item) => <Link key={item.id} href={`/inventory/${item.id}`} className="block rounded-lg px-2 py-1.5 hover:bg-paper-2"><b>{item.inventoryCode}</b> · {item.materialDescription}<span className="ml-2 text-xs text-ink-3">{item.building.buildingNumber}</span></Link>)}</Block>
    <Block title="Samples not linked to inventory" count={unlinkedSamples.length} href="/samples">{unlinkedSamples.slice(0, 6).map((sample) => <Link key={sample.id} href={`/samples/${sample.id}`} className="block rounded-lg px-2 py-1.5 hover:bg-paper-2"><b>{sample.sampleNumber}</b> · {sample.material}<span className="ml-2 text-xs text-ink-3">{sample.building.buildingNumber}</span></Link>)}</Block>
    <Block title="Duplicate internal inventory IDs" count={duplicateCodes.length} href="/inventory">{duplicateCodes.slice(0, 6).map((row) => <div key={row.internalCode} className="rounded-lg px-2 py-1.5"><b>{row.internalCode}</b> · {row._count._all} records</div>)}</Block>
  </div>;
}
