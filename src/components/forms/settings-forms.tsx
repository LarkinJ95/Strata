...
type Member = { id: string; name: string; email: string; title: string | null; phone: string | null; status: string; roleId: string };
...
export function AdministrationSettings({ laboratories, contractors, regulatoryProfiles, roles, users }: { laboratories: Laboratory[]; contractors: Contractor[]; regulatoryProfiles: RegulatoryProfile[]; roles: Role[]; users: Member[] }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      <section className="panel rounded-2xl p-5"><h2 className="mb-3 font-display text-[15px] font-semibold">Users</h2><div className="space-y-2">{users.map((member) => <Editor key={member.id} label={`Edit ${member.name} · ${roleName(member.roleId)}`}><form action={saveUser} className="space-y-3"><AccessField /><input type="hidden" name="id" value={member.id} /><div className="grid gap-3 md:grid-cols-2"><Field label="Name"><input name="name" defaultValue={member.name} required /></Field><Field label="Email"><input name="email" type="email" defaultValue={member.email} required /></Field><Field label="Title"><input name="title" defaultValue={member.title ?? ""} /></Field><Field label="Phone"><input name="phone" defaultValue={member.phone ?? ""} /></Field><Field label="Role"><select name="roleId" defaultValue={member.roleId}>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select></Field><Field label="Status"><select name="status" defaultValue={member.status}><option value="active">Active</option><option value="inactive">Inactive</option></select></Field></div><Submit label="Save user" /></form></Editor>)}<Editor label="Add user"><form action={saveUser} className="space-y-3"><AccessField /><div className="grid gap-3 md:grid-cols-2"><Field label="Name"><input name="name" required /></Field><Field label="Email"><input name="email" type="email" required /></Field><Field label="Temporary password"><input name="password" type="password" minLength={12} required /></Field><Field label="Title"><input name="title" /></Field><Field label="Phone"><input name="phone" /></Field><Field label="Role"><select name="roleId" defaultValue={roles[0]?.id}>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select></Field><input type="hidden" name="status" value="active" /></div><Submit label="Create user" /></form></Editor></div></section>
    </div>
  </div>;
}
